...
- **Residuals**: S1-S12 were not re-run live under the S13-S15 authorization (last full live suite 2026-08-12, 12/12; plugin delta since then is additive plus the exit-handler/wait/bridge paths, all covered by unit tests) — one full `S1…S15` run recommended before merge. Minor: pgrep evidence substring match; two optional spec-review nits (pre-existing §9.3 "§15 assumptions" pointer; §5 hook table omits `session.status`).
